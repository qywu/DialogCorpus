...
if __name__ == "__main__":
    ...
    with open(
        "conversations_gone_awry_corpus/data/conversations-gone-awry-corpus/utterances.jsonl"
    ) as f:
        for line in f:
            raw_turn = json.loads(line)
            all_turns.append(raw_turn)

    logger.info("Processing Conversatiosn Awry Dataset")
    ...
    all_dialogs = add_dialogue_index(
        "conversations_gone_awry_corpus", all_dialogs
    )

    # save the data
    file_path = os.path.join(
        "conversations_gone_awry_corpus/data/"
        "conversations_gone_awry_corpus.json"
    )
    logger.info("Saving Conversations Dataset")
