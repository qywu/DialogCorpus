...
if __name__ == "__main__":
    ...
    with open("friends_corpus/data/friends-corpus/utterances.jsonl") as f:
        for line in f:
            raw_turn = json.loads(line)
            all_turns.append(raw_turn)

    logger.info("Processing Friends Corpus Dataset")
    ...
    all_dialogs = add_dialogue_index("friends_corpuss", all_dialogs)

    # save the data
    file_path = os.path.join("friends_corpus/data/friends_corpus.json")
    logger.info("Saving friends_corpus Dataset")
